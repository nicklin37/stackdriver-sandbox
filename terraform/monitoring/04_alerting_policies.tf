...
# Create an alerting policy on the Availability SLO for the cartservice.
# The definition of the SLO can be found in the file '03_service_slo.tf'.
# Alert if error budget burn rate exceeds twice the allotted burn rate within the past hour. 
resource "google_monitoring_alert_policy" "cartservice_availability_slo_alert" {
  display_name = "Cart Service Availability Alert Policy"
  combiner     = "AND"
  conditions {
    display_name = "SLO burn rate alert for availability SLO with a threshold of 2"
    condition_threshold {
      ...
      # This filter alerts on burn rate over the past 60 minutes
      # Note that the service id is the Istio service that we defined the SLO on, this service id is automatically generated by Istio
      filter     = "select_slo_burn_rate(\"projects/${var.project_id}/services/ist:${var.project_id}-zone-${var.zone}-stackdriver-sandbox-default-cartservice/serviceLevelObjectives/${google_monitoring_slo.cartservice_availability_slo.slo_id}\", 60m)"
      threshold_value = "2"
      comparison = "COMPARISON_GT"
      duration   = "60s"
    }
  ...
  documentation {
    content = "Availability SLO burn for the Cart Service for the past 60m exceeded twice the acceptable budget burn rate. The service is returning less OK responses than desired. Consider viewing the service logs or custom dashboard to retrieve more information or adjust the values for the SLO and error budget."
    mime_type = "text/markdown"
  }
}

# Create an alerting policy on the Latency SLO for the cartservice.
# The definition of the SLO can be found in the file '03_service_slo.tf'.
# Alert if error budget burn rate exceeds twice the allotted burn rate within the past hour. 
resource "google_monitoring_alert_policy" "cartservice_latency_slo_alert" {
  display_name = "Frontend Service Latency Alert Policy"
  combiner     = "AND"
  conditions {
    display_name = "SLO burn rate alert for latency SLO with a threshold of 2"
    condition_threshold {
      filter     = "select_slo_burn_rate(\"projects/${var.project_id}/services/ist:${var.project_id}-zone-${var.zone}-stackdriver-sandbox-default-cartservice/serviceLevelObjectives/${google_monitoring_slo.cartservice_latency_slo.slo_id}\", 60m)"
      threshold_value = "2"
      comparison = "COMPARISON_GT"
      duration   = "60s"
    }
  ...
  documentation {
    content = "Latency SLO burn for the Cart Service for the past 60m exceeded twice the acceptable budget burn rate. The service is responding slower than desired. Consider viewing the service logs or custom dashboard to retrieve more information or adjust the values for the SLO and error budget."
    mime_type = "text/markdown"
  }
}